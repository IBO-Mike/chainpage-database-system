...
class CrashRecoveryTest {
    ...
    static Stream<org.junit.jupiter.params.provider.Arguments> points() {
        return Stream.of(
                ...
                org.junit.jupiter.params.provider.Arguments.of(
                        "allocate-table", "page_allocate", 1),
    ...
    void processDeathRollsBackWholeOperation(String action, String point, int count)
        ...
        assertTrue(Files.exists(dir.resolve("operation.undo")));
        try (StorageManager recovered = new StorageManager(dir)) {
            assertTrue(recovered.recoveredOperation());
        }
        Map<String, byte[]> after = snapshot();
        before.forEach((k, v) -> assertArrayEquals(v, after.get(k), k));
        assertFalse(Files.exists(dir.resolve("operation.undo")));
        try (StorageManager clean = new StorageManager(dir)) {
            assertFalse(clean.recoveredOperation());
        }
    }

    @Test
    void recoveryIsIdempotentAfterSecondCrash() throws Exception {
        prepare();
        Map<String, byte[]> before = snapshot();
        Process first = spawn("insert", "before_commit", 1);
        assertEquals(61, first.waitFor());
        Process second = spawn("open", "during_restore", 1);
        assertEquals(61, second.waitFor());
        assertTrue(Files.exists(dir.resolve("operation.undo")));
        try (StorageManager recovered = new StorageManager(dir)) {
            assertTrue(recovered.recoveredOperation());
        }
    ...
    private void prepare() {
        try (StorageManager s = new StorageManager(dir, 4, "LRU", true)) {
            ...
            for (int i = 0; i < 16; i++) s.indexInsert(1, i, rid(i));
            s.createTablePages("t");
            for (int i = 0; i < 2; i++) {
                int p = s.allocatePageForTable("t").get(i);
                s.writePage(p, new byte[4096], null, 0);
            }
            s.flushAll();
        }
    }
    ...
    private Map<String, byte[]> snapshot() throws Exception {
        ...
        for (String n :
                List.of(
                        ...
                        "table_pages.json",
                        "indexes.json",
                        "wal.log")) m.put(n, Files.readAllBytes(dir.resolve(n)));
        return m;
    }

    private Process spawn(String action, String point, int count) throws Exception {
        ProcessBuilder p =
                new ProcessBuilder(
                        ...
                        dir.toString(),
                        action);
        p.environment().put("CHAINPAGE_CRASH_POINT", point);
        p.environment().put("CHAINPAGE_CRASH_COUNT", Integer.toString(count));
