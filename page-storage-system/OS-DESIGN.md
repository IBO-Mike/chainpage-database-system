...
返回副本避免调用者直接修改缓存里的字节而绕过 dirty 和 WAL。LRU 的 `get` 更新访问顺序；FIFO 的读取不更新插入顺序。CLOCK 使用循环链表和引用位：访问置位，淘汰时清位并给一次第二机会；不在候选集的锁定页不会被选中。
...
`read_at/write_at` 保留直接页文件访问接口。直接写现在也取得页 WRITE 锁、先持久化 UPDATE，再使该页缓存失效、写页并同步、登记 APPLIED；之后缓存读取可见新字节。它不会装载目标页或淘汰无关页。显式 recover 先刷新 dirty 帧，完成 REDO 后清空缓存，避免干净旧副本遮住恢复结果。
...
点查沿分隔键找到叶节点，再二分搜索；范围查从起始叶沿 next 链读取，包含两端。插入只沿搜索路径修改节点，叶节点满时分裂并修复双向叶链，分隔键向上递归传播，必要时产生新根。分裂同时检查 16-key 上限和 4096 字节编码容量，支持 INT、VARCHAR 和非唯一索引。

唯一 INT 索引删除实现局部借位、合并和根收缩。删除后先刷新父分隔键，再从兄弟借位或合并，避免使用变化前的最小键。VARCHAR 和非唯一索引删除仍使用字节容量受控的整树重建作为安全回退；单 key 的 RowId overflow 页尚未实现，超过容量返回错误并由复合操作恢复。

增量节点算法减少写页，但 facade 仍会校验整树，AtomicCoordinator 仍保存全文件 undo，整体变更复杂度不能笼统宣称为 O(log N)。优化前后相同输入、相同结果的写页及耗时实验见 OS-PERFORMANCE.md。

### 崩溃恢复与复合操作

单页日志为 REDO-only。恢复按序处理未 APPLIED 的 UPDATE，跳过已释放、generation 不同或早于同代页已应用高水位的日志。新 UPDATE/APPLIED/CHECKPOINT 记录带 SHA-256，完整损坏、校验不符和序号缺口拒绝启动；无校验旧日志可继续读取，末尾未换行半记录会被截去。JSON 重复字段、null 记录、溢出或非整数元数据被拒绝。

手动 `checkpoint` 在 guard 内先 flushAll、REDO 和清缓存，再将已全部 APPLIED 的日志原子替换为带最后序号的 CHECKPOINT 记录。返回压缩前后字节数和回收量；重开从最后序号加一继续。替换前后强制终止 JVM 的测试均验证数据和序号保留。它不是通用事务 checkpoint，也没有自动周期调度。
...
5. 展示 BPlusTree 的插入分裂、INT 删除借位/合并、validate 和回退路径，解释局部写页优化与全文件 undo 的不同成本。
6. 展示 checkpoint 的前后字节数、连续序号及崩溃验证；用四种工作负载解释 CLOCK/LRU/FIFO 的差异。
